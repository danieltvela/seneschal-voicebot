#!/bin/sh
# Voicebot installer — shared library
#
# Sourced by install.sh and install-gitea.sh. Provides all installer logic
# (download, directory setup, model installation, voice picker, env writer,
# launcher writer, LLM probe, smoke test). Callers must set these globals
# before sourcing:
#
#   RELEASE_BASE           — base URL for the binary tarball
#   TARBALL                — tarball filename (e.g. voicebot-aarch64-apple-darwin.tar.gz)
#   DEFAULT_TTS_PROVIDER   — "avspeech" (macOS) or "kokoro" (Linux)
#   DEFAULT_TTS_VOICE      — macOS default voice display name (e.g. "Marisol (Enhanced)")
#   DEFAULT_KOKORO_VOICE   — Linux default Kokoro voice (e.g. "es_xb")
#   DEFAULT_KOKORO_LANG    — Linux default Kokoro language (e.g. "es")
#
# Optional globals (with defaults):
#   VOICEBOT_HOME          — install home (default: ~/.voicebot)
#   BIN_DIR                — launcher directory (default: ~/.local/bin)
#   VOICEBOT_VERSION       — release tag, "latest" (default) or pinned
#   GITHUB_REPO            — only used for messaging
#   GITEA_URL              — only used for messaging
#   WHISPER_MODEL_URL,
#   KOKORO_MODEL_URL,
#   KOKORO_VOICES_URL,
#   VAD_MODEL_URL          — overrideable model URLs
#
# This file is POSIX sh (dash-compatible). It is sourced, never executed.
# It does NOT call `set -e` — callers do that. It does NOT call `exit` on
# non-fatal errors; that decision is the caller's. The only `exit` is in
# `error()`, which signals a hard failure.
...
# ── Main orchestrator ───────────────────────────────────────────────────────
# Caller sources this file then calls `main`. We do not call main here because
# the caller may want to set globals after sourcing.
main() {
    detect_tty
    _apply_defaults
    _detect_platform

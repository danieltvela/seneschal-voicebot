...
RUST_LOG=debug STT_PROVIDER=speech exec cargo run --bin seneschal --features control,remote,speech,avspeech,tui,parakeet,remote --release -- "$@" 2> >(grep -vE "^(whisper_|ggml_)" >&2)

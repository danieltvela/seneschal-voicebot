...
RUST_LOG=debug STT_PROVIDER=speech exec cargo run --bin voicebot --features control,speech,avspeech,tui,parakeet,remote --release -- "$@" 2> >(grep -vE "^(whisper_|ggml_)" >&2)
